...
export default function Home() {
  ...
  const [isOnline, setIsOnline] = useState(navigator.onLine); // Track connectivity

  // Error Handling States
  const [orderError, setOrderError] = useState(null);
  const [balanceError, setBalanceError] = useState(null);
  const [priceError, setPriceError] = useState(null);
  const [connectivityError, setConnectivityError] = useState(null);
  ...
  useEffect(() => {
    ...
    const handleOffline = () => {
      setIsOnline(false);
      setConnectivityError('You are currently offline. Please check your internet connection.');
    };
  ...
  const handleStart = () => {
    ...
    const id = setInterval(async () => {
      try {
        ...
        if (!isOnline || !currentPrice || !minPrice || !maxPrice) return;
  ...
  return (
    <div className="container">
      ...
      {/* Connectivity Error */}
      {connectivityError && <p className="error">{connectivityError}</p>}
      ...
      <div className="buttonsContainer">
        <button onClick={handleStart} disabled={intervalId || !isOnline}>
          Start Trading
        </button>
        <button onClick={handleStop} disabled={!intervalId || !isOnline}>
          Stop Trading
        </button>
      </div>
