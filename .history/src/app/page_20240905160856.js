  ...
  useEffect(() => {
    const fetchKlinesData = async () => {
      try {
        ...
        const volumeUSDT = klinesData.reduce((acc, kline) => acc + parseFloat(kline[5]), 0); // Volume in USDT
        const volumeCrypto = klinesData.reduce((acc, kline) => acc + parseFloat(kline[7]), 0); // Volume in crypto
        ...
        setTableData((prevData) => {
          const newData = [
            ...prevData,
            {
              ...
              volumeRatio: volumeUSDT / volumeCrypto,
              price: currentPrice,
            },
          ];

          // Keep only the latest data
          return newData.slice(-100); // Adjust as needed to keep the table from growing indefinitely
        });
    ...
    fetchKlinesData();
    const interval = setInterval(fetchKlinesData, 1000); // Update every second
  ...
  return (
    <div className="container">
      ...
      <div className="table-container">
        <table className="crypto-table" ref={tableRef}>
          <thead>
            <tr>
              <th>Time</th>
          ...
          <tbody>
            {tableData.map((row, index) => (
              <tr key={index}>
                ...
                <td>{row.price ? row.price.toFixed(7) : 'N/A'}</td>
              </tr>
            ))}
          </tbody>
        </table>
